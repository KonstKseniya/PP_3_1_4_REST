...
async function getAuth() {
    try {
        ...
        alert('Error fetching authenticated user: ' + error.message);
...
async function handlerUserButton(event) {
    ...
    if (id) {
        ...
        try {
            ...
            alert('Error handling user button: ' + error.message);
...
async function handlerAddButton(event) {
    ...
    if (!validateUserData(data, 'add')) return;
...
async function handlerEditButton(event) {
    ...
    if (!validateUserData(data, 'edit')) return;
...
function validateUserData(data, actionType) {
    const errors = [];
    if (data.username.length < 3 || data.username.length > 30) {
        errors.push("Username must be between 3 and 30 characters.");
    }
    if (actionType === 'add' || data.password) {
